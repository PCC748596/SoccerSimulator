...
const gkA = gk.model.position.z * gk.dirZ;
for (const p of equipa) {
    const zz = p.model.position.z * p.dirZ;
    if (zz < gkA) console.log('ATRAS:', p.pos, p.playingStyle, 'z', zz.toFixed(1),
        'gk', gkA.toFixed(1), 'alvo', (p.dynamicTarget.z * p.dirZ).toFixed(1));
}
