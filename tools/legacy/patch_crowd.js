...
const visivelStart = code.indexOf('    setVisivel(on) {');
const visivelEnd = code.indexOf('    }', visivelStart) + 5;
...
const newBuild = `    build(scene, lugares) {
        ...
        // Isto agrupa os lugares em 'fatias' espaciais. Assim o frustum culling de um chunk funciona!
        ...
        for (let c = 0; c < numChunks; c++) {
            ...
            for (const canal of canais) {
                ...
                const m = new THREE.InstancedMesh(geos[canal], mat, nChunk);
                m.castShadow = false;
                m.receiveShadow = false;
                m.instanceMatrix.setUsage(THREE.StaticDrawUsage);
                
                // Agora SIM ligamos o frustum culling! (cada chunk tem uma BoundingBox apertada)
            ...
            for (let i = 0; i < nChunk; i++) {
                ...
                // Aumenta a box com margem para quando eles estão de pé e com os braços no ar
                minBox.x = Math.min(minBox.x, l.x - 2);
                minBox.y = Math.min(minBox.y, l.y - 2);
                minBox.z = Math.min(minBox.z, l.z - 2);
                maxBox.x = Math.max(maxBox.x, l.x + 2);
                maxBox.y = Math.max(maxBox.y, l.y + 4.5); // Altura máxima
                maxBox.z = Math.max(maxBox.z, l.z + 2);
            }
        ...
        // Retro-compatibilidade do _meshes para outras funcoes (se chamarem por ex meshes.camisa)
...
const newVisivel = `    setVisivel(on) {
        if (!this._grupos) return;
        for (const grupo of this._grupos) {
            for (const canal in grupo) grupo[canal].visible = on;
        }
    }`;

code = code.substring(0, buildStart) + newBuild + code.substring(buildEnd, visivelStart) + newVisivel + code.substring(visivelEnd);
